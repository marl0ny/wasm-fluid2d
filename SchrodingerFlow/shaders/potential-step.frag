...
uniform float m;
uniform float hbar;
uniform bool setDensityUniform;
uniform float uniformDensityAmplitude;
// uniform float q;
// uniform float c;
...
void main() {
    ...
    complex2 psi2 = multiply(psi, complex2Exp(-(0.5*iDt/hbar)*potential));
    if (setDensityUniform) {
        fragColor = uniformDensityAmplitude*psi2/complex2Length(psi2);
    } else {
        fragColor = psi2;
    }
}